...
CREATE TABLE `__new_chat_messages` (
	...
	`created_at` integer DEFAULT 1772555609230,
	`parent_id` text,
...
INSERT INTO `__new_chat_messages`("id", "role", "content", "citations", "sent_attachments", "session_id", "created_at", "parent_id", "branch_index", "path_summary", "path_last_summarized_id") SELECT "id", "role", "content", "citations", "sent_attachments", "session_id", "created_at", "parent_id", "branch_index", "path_summary", "path_last_summarized_id" FROM `chat_messages`;--> statement-breakpoint
...
CREATE TABLE `__new_chat_sessions` (
	...
	`created_at` integer DEFAULT 1772555609230,
	`updated_at` integer DEFAULT 1772555609230,
	`summary` text,
