...
export function BranchMapPanel({
  ...
  // Panel's effective top = (vh - 88 + panelY) - (DEFAULT_H + TITLE_H)
  // We need that >= margin, so panelY >= margin + 88 + DEFAULT_H + TITLE_H - vh
  useEffect(() => {
    if (minimized) return;
    ...
    const margin = 12;
    const minY =
      margin + BOTTOM_CSS_PX + DEFAULT_H + TITLE_H - window.innerHeight;
