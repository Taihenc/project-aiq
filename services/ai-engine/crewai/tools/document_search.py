...
def embedding_search(query: str, top_k: int) -> List[Dict[str, Any]]:
    """
...
def reranker(
    documents: List[Dict[str, Any]], top_n: int, top_k: int
) -> List[Dict[str, Any]]:
    """
...
class DocumentSearchTool(BaseTool):
    ...
    def _run(self, query: str, top_k: int = 10, top_n: int = 5) -> Dict[str, Any]:
        """
        ...
        Returns:
            Dict containing reranked search results with documents and scores
        """
        # Step 1: Embedding Search
        search_results = embedding_search(query, top_k)

        # If no results found
        if not search_results:
            return SearchOutput(documents=[], query=query, total=0)

        # Step 2: Rerank
        reranked_docs = reranker(search_results, top_n, top_k)
